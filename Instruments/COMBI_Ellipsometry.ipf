...
//This Procedure contains an example for a generic instrument named "Ellipsometry", this can be followed as an example for creating new vector instruments in COMBIgor
...
Static StrConstant sInstrument = "Ellipsometry"
...
//FOR DEVELOPERS: Change COMBI_Ellipsometry() to COMBI_YourInstrument() (to match the last function
...
Menu "COMBIgor"
	SubMenu "Instruments"
	 	"Ellipsometry",/Q, COMBI_Ellipsometry()
	end
end
...
Function/S Ellipsometry_Descriptions(sGlobalName)
	...
	strswitch(sGlobalName)//depending on value of sGlobalName
		...
		case "OnAccessPanel":
			...
			break
		case "Ellipsometry":
			sReturnstring = "Ellipsometry"
			break
		//these are the specific variables that are collected by the Define process
		case "sAbsorption":
			sReturnstring =  "Absorption coefficient:"
			break
		case "sWavelength":
			sReturnstring =  "Wavelength:"
			break
		case "sEnergy":
			sReturnstring = "Energy:"
			break
		default:
			sReturnstring =  ""
			break
...
//with your own specific globals for your instrument. This function uses many COMBIgor Basics functions (ie the Combi_Prompt
//functions and the Combi_GetInstrument functions), so be sure to check those out and double check that you are passing them 
//the right inputs.
function Ellipsometry_Define()
	...
	if(!stringmatch("NAG",COMBI_GetInstrumentString(sThisInstrumentName,"sProject",sProject)))
		...
		sAbsorption = COMBI_GetInstrumentString(sThisInstrumentName,"sAbsorption",sProject)
	...
	sAbsorption = cleanupname(COMBI_DataTypePrompt(sProject,sAbsorption,Ellipsometry_Descriptions("sAbsorption"),0,1,0,2),0)
	//if cancel was selected -- break
	if(stringmatch(sAbsorption,"CANCEL"))
		return -1
	endif
	//cleanupname makes sure the data type name is allowed
	sWavelength = cleanupname(COMBI_DataTypePrompt(sProject,sWavelength,Ellipsometry_Descriptions("sWavelength"),0,1,0,2),0)
	//if cancel was selected -- break
	if(stringmatch(sWavelength,"CANCEL"))
		return -1
	endif
	//cleanupname makes sure the data type name is allowed
	sEnergy = cleanupname(COMBI_DataTypePrompt(sProject,sEnergy,Ellipsometry_Descriptions("sEnergy"),0,1,0,2),0)
	//if cancel was selected -- break
	...
	COMBI_GiveInstrumentGlobal(sThisInstrumentName,"sAbsorption",sAbsorption,sProject)// store Instrument global
...
function Ellipsometry_Load()

	//initializes the instrument
	Combi_InstrumentReady("Ellipsometry")
	...
	//Combi_GiveInstrumentGlobal("Ellipsometry","vSomeNumber",num2str(vSomeNumber),sProject)
	//if the user is loading an entire folder, call the LoadFolder function
	if(stringmatch(sLoadType,"Folder"))
		// get globals
		string sAbsorption = Combi_GetInstrumentString("Ellipsometry","sAbsorption",sProject)
		string sWavelength = Combi_GetInstrumentString("Ellipsometry","sWavelength",sProject)		
		string sEnergy = Combi_GetInstrumentString("Ellipsometry","sEnergy",sProject)		
		...
		if(stringmatch("Yes",Combi_GetGlobalString("sCommandLines","COMBIgor")))
			Print "Ellipsometry_LoadFolder(\""+sProject+"\",\""+sAbsorption+"\",\""+sWavelength+"\",\""+sEnergy+"\")"
		endif
		//call the LoadFolder function
		Ellipsometry_LoadFolder(sProject,sAbsorption,sWavelength,sEnergy)
	//if the user is just loading a single file, call the LoadFile function
	elseif(stringmatch(sLoadType,"File"))
		Ellipsometry_LoadFile()
	endif
...
function Ellipsometry_LoadFolder(sProject,sAbsorption,sWavelength,sEnergy)
	//define strings and variables	
	string sProject, sAbsorption, sWavelength, sEnergy
	//get users current folder to return to
	string sTheCurrentUserFolder = GetDataFolder(1) 
	setdatafolder root:
	variable vIndex
	string sThisInstrumentName = "Ellipsometry"
	...
	for(iSample=(vFirstFileNum-1);iSample<(vLastFileNum);iSample+=1)
		...
		sWaveName = "LoadedWave_"+COMBI_PadIndex((iSample+1),0)
		//print sWaveName
		//sets the source file name based on previous user inputs, correcting for constant index length
		sfileName = sDataFileNamePrefix+COMBI_PadIndex((iSample+1),0)+sDataFileNameSuffix+".txt"
		//print sfileName
		...
		LoadWave/G/M/O/D/N=$sWaveName/p=pLoadPath/K=1/Q/L={0,2,0,0,2} sfileName
		...
		wave wWaveIn = $sWaveName
		redimension/N=(-1,3) wWaveIn
		wWaveIn[][2]= 1240/wWaveIn[p][0]
	endfor
	setdatafolder root:
	...
	Combi_SortNewVectors(sProject, "root:ScansFolder:","LoadedWave_","",0,sNewLibrary,sWavelength+";"+sAbsorption+";"+sEnergy,num2str(vFirstSample-1),num2str(vFirstSample+vNumberOfFiles-2),num2str(vFirstFileNum-1),num2str(vLastFileNum-1),0)
	...
	sLogEntry4 = "Data Types: "+sAbsorption+","+sWavelength
	sLogEntry5 = ""
	sLogText = sLogEntry1+";"+sLogEntry2+";"+sLogEntry3+";"+sLogEntry4+";"+sLogEntry5
	Combi_Add2Log(sProject,sNewLibrary,"Ellipsometry",1,sLogText)			
...
function/S Ellipsometry_LoadFile()
	
	// get import path global
	string sLoadPath = Combi_GetGlobalString("sImportOption","COMBIgor")
	
	string LoadName = "Ellipsometry"
	...
	if(stringmatch("Import Folder",sLoadPath))
		...
		LoadWave /P=pUserPath /A=$LoadName /J /L={0,2,0,0,2} 
...
//FOR DEVELOPERS: Change "Ellipsometry" to the name of your Instrument; make sure your loader is appropriately named COMBI_YourInstrument.ipf
function COMBI_Ellipsometry()
	COMBI_GiveGlobal("sInstrumentName","Ellipsometry","COMBIgor")
	COMBI_InstrumentDefinition()
	//DisplayProcedure/W=$"COMBI_Ellipsometry.ipf"/L=0
end
